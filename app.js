...
(function () {
  ...
  function init() {
    ...
    var aboutVideo = document.querySelector("#estrutura .main-photo");
    if (aboutVideo && aboutVideo.tagName === "VIDEO") {
      ...
      var unlockVideoAudio = function () {
        aboutVideo.muted = false;
        var resume = aboutVideo.play();
      ...
      var tryAutoplayWithSound = aboutVideo.play();
      if (tryAutoplayWithSound && typeof tryAutoplayWithSound.catch === "function") {
        tryAutoplayWithSound.catch(function () {
          // Most browsers block autoplay with sound until user interaction.
          aboutVideo.muted = true;
          var mutedPlay = aboutVideo.play();
          if (mutedPlay && typeof mutedPlay.catch === "function") {
            mutedPlay.catch(function () {});
          }
          window.addEventListener("pointerdown", unlockVideoAudio, { passive: true, once: true });
          window.addEventListener("keydown", unlockVideoAudio, { passive: true, once: true });
          window.addEventListener("touchstart", unlockVideoAudio, { passive: true, once: true });
        });
      }
    }
    ...
    Array.prototype.forEach.call(document.querySelectorAll(".review-carousel"), function (carousel) {
      ...
      function update() {
        var cols = getCols();
        pageCount = Math.max(1, cards.length - cols + 1);
        if (page >= pageCount) page = 0;

        var cardWidth = cards[0].getBoundingClientRect().width;
        var gap = parseFloat(window.getComputedStyle(trackEl).gap || "0");
        var offset = page * (cardWidth + gap);
